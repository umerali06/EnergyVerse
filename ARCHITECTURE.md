...
Phases 0, 1, 3, 4, 5, 6, 7, and 8 are complete; every currently defined Phase 2
subphase is also done. FastAPI publishes a committed OpenAPI contract, pinned
generators produce typed TypeScript and Dart clients, and both applications consume
those clients through shared token/error/feedback wrappers. Phase 9 is AI report
generation; its policy contract and backend foundation are complete, and private
PDF/DOCX/XLSX rendering is next but not started. Detailed feature architecture remains intentionally
...
- Next.js maps the generated values into Tailwind and CSS custom properties. Its
  theme provider defaults to light, persists the user's light/dark choice, and its
  Framer Motion helpers honor `prefers-reduced-motion`.
- Flutter builds light-default and dark `ThemeData` from generated Dart constants,
...
### Phase 9.1 AI Report Policy and Domain Contract

- The five MVP report types come directly from source brief §13: inspection,
  maintenance, safety, executive summary, and asset health.
- A report owns a tenant-scoped snapshot of its source data. Later changes to an
  inspection, work order, safety report, asset, or dashboard aggregate do not
  silently rewrite an existing report.
- Draft creation requires `reports.generate` plus the corresponding source read
  permission. The tenant report library requires `reports.read`; neither grants
  cross-tenant access.
- AI-written narrative is advisory and editable. Inspection findings included in
  a report must already satisfy the Phase 7.10 human-review gate before the draft
  can be finalized.
- Human finalization is explicit and records authenticated reviewer identity,
  timestamp, and attestation. A finalized snapshot is immutable; only drafts may
  be regenerated, edited, or deleted.
- PDF, DOCX, and XLSX files are deterministic renderings of the finalized
  snapshot, private in Firebase Storage under
  `companies/{company_id}/reports/{report_id}/exports/`, and exposed only through
  fresh request-time signed URLs in accordance with D-027.
- Phase decomposition: 9.2 builds the persisted domain/generation/finalization
  API and typed contracts; 9.3 renders and stores all three export formats; 9.4
  ships the admin/mobile workflow; 9.5 integrates the real dashboard count and
  closes the phase with full regression verification.

### Phase 9.2 Report Backend Foundation

- `GeneratedReport` is a strict tenant document in `generated_reports` with a
  draft/finalized lifecycle, monotonic revision, complete JSON-compatible source
  snapshot, structured advisory narrative, model provenance, and authenticated
  finalizer provenance.
- Seven `/api/v1/reports` operations list, generate, read, human-edit,
  regenerate, finalize, and soft-delete drafts. Reads require `reports.read`;
  mutations require `reports.generate`; generation/regeneration additionally
  checks the corresponding source permission from D-083.
- Snapshot assembly supports inspection, maintenance/work-order, safety,
  executive-summary, and asset-health reports. Single-source records are loaded
  through scoped repositories; executive and asset-health aggregates use the
  established 5,000-record bounded-query convention.
- `ReportNarrativeClient` is the provider-neutral test seam. The production
  `ClaudeReportNarrativeClient` uses forced structured tool output and refuses to
  operate without configuration; provider or schema failures become the stable
  502 `report_generation_failed` envelope.
- Regeneration atomically replaces a draft's snapshot, source revision,
  narrative, and model provenance under an expected-revision guard. Finalization
  requires literal attestation, stamps authenticated identity/time, rejects any
  included unreviewed inspection analysis, and permanently blocks edit,
  regeneration, and deletion.
- Repository writes emit complete before/after audit events. OpenAPI and pinned
  TypeScript Fetch/Dart Dio clients expose the full typed surface.

### Phase 9.3 Private Report Exports

- `POST /api/v1/reports/{report_id}/export?format=pdf|docx|xlsx` requires
  `reports.read`, loads the report through the tenant scope, and rejects drafts.
- Deterministic server renderers create branded PDF, DOCX, and XLSX bytes from
  only the immutable finalized snapshot, narrative, and stored provenance. The
  Word/PDF outputs are paginated business reports; Excel separates the readable
  summary from a filterable authoritative-source sheet.
- `GeneratedReportStorage` writes only to the fixed private path
  `companies/{company_id}/reports/{report_id}/exports/{filename}`. The persisted
  aggregate stores artifact metadata and generator provenance, never a download
  URL; every response creates a fresh request-time signed URL.
- Re-exporting a format replaces that format's artifact metadata without
  changing the finalized report revision. Every successful export emits a
  `generated_report.exported` before/after audit event.
- OpenAPI and both pinned clients expose the export operation and response.
  Structural parsing plus page-by-page visual renders verify all three formats.

### Phase 9.4a Admin Report Library and Downloads

- The protected `/reports` route replaces its placeholder with a tenant report
  register driven exclusively by the generated `GeneratedReportsApi` through the
  shared authenticated `FevApiClient` boundary.
- Type and status filters restart cursor pagination; loading, retry, empty, and
  load-more states remain local to the report module. Navigation already requires
  `reports.read`, and FastAPI independently enforces the same permission.
- Draft rows state that finalization is required and expose no export control.
  Finalized rows request PDF, DOCX, or XLSX on demand, then immediately follow the
  fresh signed URL returned by the server; no URL is cached or persisted in UI
  state. Success/failure uses the shared toast and each action has an accessible
  report-and-format label.

### Phase 9.4b.1 Admin Report Creation

- `/reports/new` requires the UI's `reports.generate` capability before loading
  any data. Inspection, maintenance, safety, and asset-health choices additionally
  require their D-083 source read permission and load the matching tenant list;
  executive summaries load no source and submit none.
- The form sends a browser-generated opaque report ID, selected report type,
  optional human title, and exact source ID through the generated create contract.
  The server remains authoritative for snapshot assembly, source validation,
  narrative generation, AI model provenance, tenancy, RBAC, and audit history.
- Source loading, permission denial, empty validation, generation progress, and
  provider failure are explicit states. Successful creation returns to the real
  library until the detail workflow lands in 9.4b.2, avoiding broken navigation.

### Phase 9.4b.2 Admin Report Review and Finalization

- Protected `/reports/{id}` loads the complete tenant detail through the generated
  client and renders narrative/model/revision provenance plus a flattened,
  readable view of the frozen authoritative source snapshot.
- Users with `reports.generate` may edit title, summary, line-oriented findings
  and recommendations, and risk score while the report is a draft. Every request
  carries the loaded revision; rejection reloads authoritative state instead of
  preserving a false optimistic result.
- Regeneration has an explicit destructive-edit confirmation and re-reads current
  source data server-side. Finalization remains disabled until the human checks a
  review attestation, then calls the terminal server transition. Finalized detail
  is visibly immutable and exposes no mutation controls.
- Draft deletion requires a separate confirmation and returns to the library only
  after the audited server deletion succeeds. Read/generate route gates align with
  their API permissions; tenant isolation and reviewed-inspection-AI enforcement
  remain authoritative backend rules.

### Phase 9.4c.1 Mobile Report Library and Downloads

- The mobile Reports destination is no longer a placeholder. `AppRoutes` wraps
  the real `ReportsScreen` in the established auth shell and a `reports.read`
  permission gate; FastAPI independently enforces the same boundary.
- `GeneratedReportsApiContract` is a narrow generated-client seam implemented by
  `ApiService`, avoiding new methods on the broad `ApiContract` and therefore
  preserving unrelated offline/test implementations. The controller owns type
  and status filters, cursor pagination, retry, and per-artifact progress.
- Draft cards show a finalization requirement and expose no export controls.
  Finalized PDF/DOCX/XLSX actions request a fresh signed URL and pass it directly
  to a `url_launcher`-backed opener; the URL is neither cached nor persisted.
  The opener is injectable so widget tests prove the exact signed URL without
  invoking an external platform application.
...
| Mobile generated-query transport compatibility maintenance | The generated Dart/Dio clients include unset nullable query keys with empty-string values. The shared authenticated `ApiService` interceptor removes only those empty transport artifacts before dispatch, so optional datetime filters do not produce FastAPI 422 responses and optional string filters remain truly absent. `GET /api/v1/assets` also normalizes blank filter values defensively before calling `AssetManagementService`, preventing an accidental Firestore `facility_id == ""` query while preserving tenant scope, RBAC, real filters, sorting, and pagination. | 2026-08-26 |
| Mobile asset-detail refresh-safe routing maintenance | Asset rows navigate to `/assets/detail?id={asset_id}` while retaining the existing route argument for backward compatibility. `AppRoutes` parses the URL path for route matching and resolves the ID from either source, so Flutter web refresh/deep links reconstruct `AssetDetailScreen` without unsafe null casts. A missing ID renders the authenticated branded 404 inside the Assets shell. | 2026-08-26 |
| Flutter web Firebase SDK self-hosting maintenance | `web/firebase/` vendors the App/Auth/Storage ESM builds at FlutterFire's exact supported Firebase JS SDK version (12.15.0 for the current lockfile). `web/index.html` imports and registers those namespaces before injecting `flutter_bootstrap.js`, so FlutterFire detects initialized SDK modules and skips its runtime `gstatic.com` imports without emitting a version-compatibility warning. Firebase Auth/Storage network operations and the provider-neutral backend verification boundary are unchanged. | 2026-08-26 |
| Work Order list index-readiness maintenance | `infra/firebase/firestore.indexes.json` now contains Work Order composites for tenant-only and tenant plus `asset_id`, `facility_id`, or `status`, each ordered by `created_at DESC`. `WorkOrderRepository.query` uses those ordered queries normally and falls back to the same bounded filtered stream plus local descending sort used by the Asset repository when an index is unavailable/building, preventing asset-detail Work Orders from surfacing HTTP 500. | 2026-08-26 |
| Asset Media real-camera capture maintenance | The Media tab's Camera action now pushes a photo-only `PhotoCameraCaptureScreen` backed by the existing `camera` plugin preview/controller and uploads the returned `XFile` only after `takePicture()` completes. The reusable capture view hides video controls in this mode. Gallery continues through `ImagePicker.gallery`, so camera and file selection are distinct on web and native platforms. | 2026-08-26 |
| Camera initialization recovery maintenance | The shared camera view now models initializing, ready, and failed states explicitly. Browser `NotAllowedError`, missing-device, busy-device/`NotReadableError`, and unsupported-constraint failures map to stable operator guidance; failure stops the loader and exposes `Retry camera`, which disposes any prior controller and performs a clean re-enumeration/initialization. | 2026-08-26 |
...
| Phase 5.1 — safety-report data model and backend lifecycle | Adds `SafetyReport` as a strict `TenantDoc` in the `safety_reports` collection with the nine source-brief categories, four severity levels, server-derived reporter, occurrence time/GPS, manager assignment, lifecycle timestamps, soft deletion, and monotonic revision. Seven `/api/v1/safety-reports` operations provide create/list/get, HSE assignment and controlled transition, close, and delete. Firestore reads are tenant-bound and capped at 5,000 records, with one equality filter pushed down and remaining filters applied to that bounded set. D-068's dedicated `safety.close` permission separates reporting from incident control. Every mutation writes a before/after audit event. OpenAPI and both pinned clients expose `SafetyReportsApi` and typed request/response models. Evidence/corrective-action records and client UI remain explicitly scoped to 5.2/5.3. | 2026-08-16 |
| Phase 5.2 — safety evidence and corrective actions | Extends each `SafetyReport` with typed `SafetyEvidence[]` and `CorrectiveAction[]`. Evidence bytes use D-027's server-mediated private-storage boundary through `SafetyEvidenceStorage`, with strict MIME/size validation, cleanup if Firestore persistence fails, fresh signed URLs on every detail response, uploader-or-HSE deletion, and closed/cancelled immutability. Corrective actions carry assignee/due date/priority plus server-stamped start/completion/cancellation accountability; the service verifies that assignees are active users in the same tenant and enforces self-only field updates independently of route permissions. Five new routes add/delete evidence and create/update/cancel actions. The repository resolution gate refuses `corrective_action → resolved` without at least one action or while any non-cancelled action remains incomplete. All array mutations rewrite a fully validated report and emit before/after audit events. Contracts expose the expanded detail plus evidence/action models and requests. | 2026-08-16 |
| Phase 5.3a — admin HSE safety interface | Replaces `/safety`'s `ComingSoonScreen` with a responsive list/detail control center using only the shared design primitives and typed `SafetyReportsApi`. The list supports server-backed status/severity filtering and honest loading/error/empty states. Authorized reporters get the real incident form and evidence uploader; `safety.close` holders get manager assignment, ordered lifecycle transitions, close, evidence removal, corrective-action assignment, completion, and cancellation controls. Detail rendering uses fresh signed evidence links and the backend's authoritative revision/status fields; protected route metadata remains noindex through the existing SEO helper. `FevApiClient`/`AuthContext` expose all eleven safety operations rather than feature-local fetch calls. Mobile is deliberately separate 5.3b so its durable local queue and conflict behavior receive an independent test gate. | 2026-08-16 |
| Phase 5.3b.1 — mobile safety offline foundation | Adds a focused `SafetyApiContract`, schema-v11 `LocalSafetyReports` cache and FIFO `SafetyOutbox`, `LocalSafetyReportsRepository`, and `SafetySyncEngine`. Report creation is local-first and atomic: a client UUID, normalized real operator input, occurrence time/GPS, and serialized generated-contract request are committed with the create mutation in one SQLite transaction. Replay uses exponential backoff and manual bypass; a duplicate client ID is reconciled by fetching that exact tenant-scoped report, while unreadable duplicates and network failures retain the local content. Safety data has its own authenticated-user ownership key and is wiped on account changes, preventing cached cross-user exposure. Binary evidence is intentionally excluded from this lightweight queue and remains 5.3b.3 work. | 2026-08-16 |
| Phase 5.3b.2 — mobile safety reporting screens | Replaces the mobile Safety placeholder with guarded `/safety` and `/safety/detail` routes. `SafetyReportsScreen` is backed entirely by the schema-v11 cache, supports status/severity filtering, exposes sync state, and only renders incident creation for `safety.write`; the server remains the authorization boundary. The incident form commits title/description, generated-contract category/severity, occurrence date, and optional real GPS through `createOffline`, then opens the local detail immediately and kicks background synchronization. `SafetyReportDetailScreen` renders the cached description, reporter/manager, coordinates, revision, lifecycle, and retryable sync error without requiring connectivity. Evidence and corrective-action interaction remain explicitly scoped to 5.3b.3. | 2026-08-16 |
| Phase 5.3b.3 — mobile safety evidence and assigned actions | Advances Drift to schema v12 with a dedicated `SafetyEvidenceQueue` and nullable corrective-action targets on `SafetyOutbox`. Camera/gallery photo/video files are copied from temporary picker paths into tenant-user-local application documents, sanitized, validated against actual bytes and the backend's MIME/10 MiB photo/250 MiB video limits, and removed only after confirmed upload or explicit discard. Evidence replay is withheld until its parent client-generated report is synced, then uses the server-mediated multipart endpoint, bounded retry/backoff, permanent-error pause/retry UX, and authoritative response replacement. Assigned users can optimistically start or complete their own cached corrective actions (completion requires notes); FIFO replay targets the exact action and replaces optimistic state with the server snapshot. Duplicate report IDs reconcile by exact fetch, while permanent action/transition failures remain visible as report sync errors with manual retry instead of being discarded. `SafetyEvidenceProvider` and its worker are wired through authentication, resume, and disposal; user changes wipe report, mutation, and evidence queues together. | 2026-08-19 |
| Phase 5.4 — safety dashboard integration | Adds `SafetyReportRepository.count()` and `SafetyReportService.get_dashboard_summary()` behind the `safety.read`-gated `/api/v1/dashboard/safety-summary` route. The response contains the active-report total plus all nine canonical category counts, implemented with tenant-scoped Firestore aggregations and `deleted_at == null`; report documents are never downloaded. Generated TypeScript/Dart contracts feed two isolated widgets on each client: Safety Incidents opens the real report list, while Safety Incidents by Type renders the canonical breakdown with loading/empty/error behavior. The former reserved Safety tile is removed. Authenticated admin routes retain the established noindex/robots policy. Phase 5 is **COMPLETE**. | 2026-08-19 |
| Phase 6.1 — Permit-to-Work policy contract | Locks the safety-policy boundary before schema work: 5×5 initial/residual risk, versioned tenant checklist templates snapshotted per permit, sequential configurable approvals (Operations Manager → HSE Manager default), server-derived issuer/approver/worker signatures, mandatory validity and automatic expiry, worker acknowledgement before activation, and explicit suspended/revoked exceptional states. Worker acknowledgement/signature will be offline-capable; approval/activation will be online-only. `permits.write` will be distinct from `permits.approve`. No application schema or endpoint was created in this decision-only slice. | 2026-08-19 |
| Phase 6.2a — versioned tenant permit templates | Adds the `permit_templates` aggregate and repository/service/router boundary. Each tenant-owned template embeds typed checklist items and ordered approval steps that reference tenant role IDs, allowing custom approval roles without schema changes. Write routes require the new `permits.write`; reads retain `permits.read`. Server-assigned aggregate/nested IDs, soft deletion, scoped role validation, audit before/after state, cursor pagination, and required `expected_version` conflict handling protect integrity and concurrency. OpenAPI plus pinned TypeScript/Dart clients expose the five CRUD operations; permit records will consume immutable snapshots in 6.2b. | 2026-08-19 |
| Phase 6.2b — permit record foundation | Adds the tenant-scoped `permits` aggregate with five CRUD routes behind `permits.read`/`permits.write`. Creation resolves the selected facility/optional area/optional asset and every active worker inside the caller's tenant, generates an opaque ID plus human `PTW-YYYYMMDD-XXXXXXXX` number, calculates both initial and residual 5×5 risk score/band server-side, and freezes the selected template's identity/version/checklist/approval chain into the permit. Draft PATCH permits only title, description, validity, risk, and worker changes and requires `expected_revision`; the repository protects identity, location, template snapshot, lifecycle, and permit number. Lists support opaque cursor pagination plus permit-type/facility/worker filters; soft deletion and full before/after audit history follow existing tenant repository conventions. | 2026-08-19 |
| Phase 6.3a — controlled submission and sequential approval | Adds `POST /permits/{id}/submit` behind `permits.write` and `POST /permits/{id}/approval-decision` behind `permits.approve`. Submission validates revision/status/validity/residual risk and exact required checklist completion, then persists checklist completion provenance plus a server-derived issuer signature. Approval resolves the actor's active tenant user to its role ID and consumes the next required snapshotted step only; each decision carries authenticated signer/time, the last approval advances to `pending_signatures`, and a reasoned rejection returns to `draft` for correction. Repository transitions revision-guard the expected source status and write complete before/after audits. | 2026-08-19 |
| Phase 6.3b — worker acknowledgement and activation | Adds `POST /permits/{id}/acknowledge` behind `permits.read` plus assignment enforcement, and `POST /permits/{id}/activate` behind `permits.write`. Acknowledgements persist server-derived signer/receipt/signature provenance alongside client mutation/time/device metadata. Same-mutation replay returns current state without a write; different stale mutations receive the shared revision-conflict envelope for explicit client rebase. The activation boundary remains online-only and requires all snapshotted assigned workers, a currently open validity window, and the expected revision before moving `pending_signatures` → `active` with server-derived activator/time. Full before/after audit events cover both transitions. | 2026-08-19 |
| Phase 6.3c — expiry and controlled terminal lifecycle | Adds reasoned `suspend`/`resume`/`revoke`/`close` operations behind `permits.approve`, with source-state and expected-revision enforcement plus actor/time/reason or closeout provenance. Revoked and Closed are terminal; Suspended can return to Active only inside validity. The service reconciles any due Pending Approval/Pending Signatures/Active/Suspended permit to Expired before list/detail/transition results, while `scripts/expire_permits.py` provides the recurring all-tenant scheduler entry point. Automatic transitions use the same repository revision/audit boundary with `system:permit-expiry`. Generic PATCH and DELETE now reject every non-Draft permit, closing a lifecycle bypass found during this slice. | 2026-08-19 |
| Phase 6.4a — admin permit-template management | Extends the authenticated `FevApiClient` and auth-context capability surface with the generated `PermitTemplatesApi`, then replaces `/permits`' placeholder with a real filterable/paginated template list. `/permits/templates/new` and `/permits/templates/[id]` share a responsive builder for identity, ordered checklist controls, and sequential approval steps sourced from the real tenant role catalog. Create/update/delete use generated request types, optimistic template versioning, toasts, and explicit conflict/error/no-access states. Read-only roles receive list access without mutation affordances; every route retains protected-page SEO metadata. | 2026-08-19 |
| Phase 6.4b.1 — admin permit register and draft creation | Connects the authenticated admin client to the generated `PermitsApi`. `/permits` renders the tenant's cursor-paginated permit register with permit-type/facility filters and resolved facility names; template management moves to `/permits/templates`. `/permits/new` composes a server-authoritative draft from live active facilities/users, type-matched templates, facility areas/assets, local-time validity inputs converted to instants, assigned workers, and one or more 5×5 risk rows. Client validation prevents malformed or increasing residual-risk submissions while the backend remains authoritative for scores, bands, tenant relationships, and lifecycle rules. `permits.write` gates all creation resource reads and controls. At this slice boundary creation returned to the register to avoid an absent/placeholder detail route; 6.4b.2 now supersedes that temporary navigation with the real detail route. Protected SEO metadata covers each route. | 2026-08-19 |
| Phase 6.4b.2 — admin permit detail and lifecycle controls | Adds the protected dynamic `/permits/[id]` surface and generated-client wrappers for submit, approval decision, activation, suspend, resume, revoke, and close. The screen renders the immutable template/checklist/approval snapshots, server-calculated risk scores, validity, revision, and worker acknowledgement progress from `PermitDetail`. Draft issuers select checklist completions and explicitly attest before submission; approvers act on the displayed next sequential step and rejection requires a reason; activation remains disabled until every assigned worker has acknowledged. Active/Suspended states expose only their valid reasoned/attested exceptional transitions. Every call carries the currently loaded revision, relies on server-derived signatures/audit, and reloads authoritative detail after any rejection/conflict. `permits.write` and `permits.approve` separately gate controls while `permits.read` protects the route. Register rows and creation success now navigate to the real detail route. | 2026-08-19 |
| Phase 6.4c — mobile offline permit workflow | Adds schema-v13 `LocalPermits` and `PermitOutbox` Drift tables plus a dedicated repository/sync engine rather than coupling permit signatures to another domain queue. Sign-in refreshes every cursor page for the authenticated worker, stores complete generated `PermitDetail` snapshots, removes no-longer-assigned synced records, and wipes both tables on UID changes. Explicit worker attestation atomically queues an identity-stable acknowledgement; connectivity-aware FIFO replay provides transient backoff, permanent-error pause/manual retry, exact-mutation already-applied recovery, and full-server-snapshot conflict state. Discard accepts the server version; rebase changes only the expected revision while retaining the original mutation ID, signed time, and device provenance. The `permits.read`-gated mobile list/detail render cached scope, checklist, risk, approval, acknowledgement, pending/error/conflict states, manual sync, and explicit resolution without synthetic runtime data. | 2026-08-20 |
| Phase 6.5 — Active Permits KPI and Permit-to-Work closure | Adds `PermitDashboardSummary` and `GET /api/v1/dashboard/permits-summary` behind `permits.read`. `PermitService.get_dashboard_summary` tenant-loads non-deleted permits, applies the existing server-time expiry transition/audit boundary, and counts only records still Active, preventing overdue permits from inflating the operational KPI. Regenerated TypeScript Fetch and Dart Dio clients feed new independently loading/retrying `permits.active` widgets in admin and mobile; both remove the old reserved permit placeholder and navigate to the real permit register. General dashboard access remains `reports.read`, while the widget endpoint and registry visibility separately require `permits.read`. Full cross-application regression and the optimized admin build close Phase 6. | 2026-08-20 |
| Phase 9.1 — AI report policy and domain contract | Selects AI report generation as the next MVP capability and locks D-083 before schema work. Reports are tenant-scoped immutable-at-finalization snapshots; draft creation requires `reports.generate` plus source-domain read authority; AI narrative remains advisory/editable; inspection AI must be human-reviewed before finalization; and PDF/DOCX/XLSX exports remain private under D-027 with fresh signed downloads. The phase is decomposed into backend foundation, export rendering/storage, client workflow, and dashboard closure slices. No runtime schema, endpoint, or placeholder behavior is introduced by this decision-only gate. | 2026-08-20 |
| Phase 9.2 — report backend foundation | Adds the tenant-scoped `GeneratedReport` aggregate, structured advisory narrative, complete source snapshots for all five §13 report types, and seven typed `/api/v1/reports` operations. `reports.generate` is combined with the source domain's read permission at generation/regeneration; report reads remain `reports.read`. Draft edits and regeneration are revision-safe and audited. Explicit attested finalization stamps authenticated reviewer provenance, blocks snapshots containing unreviewed inspection AI, and makes the report immutable. `ReportNarrativeClient` isolates the forced-tool Claude adapter from tests. Pinned OpenAPI TypeScript/Dart clients are regenerated; artifacts remain correctly deferred to 9.3. | 2026-08-20 |
| Phase 9.3 — private report exports | Adds deterministic PDF/DOCX/XLSX rendering from immutable finalized snapshots, fixed tenant-private object paths, persisted artifact/generator provenance, audited re-export, and fresh request-time signed downloads. Draft export is rejected; stored and API-visible report data never contains a durable download URL. Structural tests parse every format, and page-by-page visual QA verifies the native PDF plus LibreOffice-rendered Word and Excel output. OpenAPI and pinned TypeScript/Dart clients are regenerated. | 2026-08-20 |
| Phase 9.4a — admin report library and downloads | Replaces the protected reports placeholder with a generated-client tenant register, type/status filtering, cursor pagination, complete loading/error/empty states, and finalized-only PDF/DOCX/XLSX actions. Each action requests a fresh signed URL and initiates the download without retaining it. Navigation and API permissions remain aligned at `reports.read`; focused interaction tests cover real data, draft gating, filter requests, and signed-download initiation. | 2026-08-20 |
| Phase 9.4b.1 — admin report creation | Adds `/reports/new` with `reports.generate` gating, the exact per-report source permission matrix, real tenant inspection/work-order/safety/asset selectors, executive-summary no-source behavior, optional human title, and generated-client draft creation. The server still owns snapshot assembly and AI narrative generation. Focused tests cover source payloads, executive-summary semantics, and zero source reads without generation permission. | 2026-08-20 |
| Phase 9.4b.2 — admin report review and finalization | Adds protected report detail with frozen-source visibility, model/revision provenance, revision-safe human narrative edits, explicit regeneration confirmation, mandatory human finalization attestation, immutable finalized rendering, and confirmed draft deletion. Mutation rejection reloads server state. Combined report tests cover every transition, and the generated client remains the only transport boundary. | 2026-08-20 |
| Phase 9.4c.1 — mobile report library and downloads | Replaces the mobile Reports placeholder with an authenticated, `reports.read`-gated generated-client module. The controller supplies type/status filters, cursor pagination, retry, and export progress. Drafts cannot export; finalized format actions request a fresh signed URL and immediately open it through an injectable `url_launcher` seam without persistence. Focused controller/widget and shell-routing tests pass. | 2026-08-20 |
| Phase 9.4c.2 — mobile source-aware creation and report review/finalization workflow | Adds `ReportCreateSheet` (source-aware creation bottom sheet) and `ReportDetailScreen` (mobile report detail screen) to `apps/mobile/lib/reports/`. Supports creation of reports across all 5 types with source ID inputs, editing draft human summary & recommendations, regenerating AI advisory narrative, mandatory human finalization attestation modal, draft deletion, and private signed-URL downloads for finalized reports. 279 passing mobile tests. | 2026-08-22 |
| Phase 9.5 — Reports Generated dashboard KPI and Phase 9 regression verification | Adds `ReportDashboardSummary` and `GET /api/v1/dashboard/reports-summary` behind `reports.read`. `ReportService.get_dashboard_summary` counts total, draft, and finalized generated reports across the caller's tenant. Regenerated TypeScript Fetch and Dart Dio clients feed new independently loading/retrying `reports.generated` widgets in admin and mobile dashboards, removing the old reserved reports placeholder. Full system regression passed with 433 pytest, 208 vitest, and 280 flutter tests. Phase 9 is **COMPLETE**. | 2026-08-22 |
| Phase 10.1 — 3D Scene & Hotspot Data Contract and Backend Foundation | Adds 3D Digital Twin models (`DigitalTwinSceneResponse`, `DigitalTwinHotspotResponse`, `UpdateDigitalTwinSceneRequest`) in `models/api.py`. Implements `get_facility_3d_scene` and `update_facility_3d_scene` in `FacilityManagementService` backed by Firestore collection `digital_twin_scenes`, synthesizing default camera presets (`Overhead Overview`, `Pump Skid Station`, `Tank Farm Area`) and spatial grid placement for assets, enriching hotspots with live status indicators (`Healthy`/`Warning`/`Critical`). Adds endpoints `GET` and `PUT` `/api/v1/facilities/{facility_id}/3d-scene` gated by `facilities.read`/`facilities.write` with audit logging (`digital_twin_scene.updated`). 5 Pytest unit tests passed. Regenerated OpenAPI spec, TypeScript Fetch, and Dart Dio contracts. | 2026-08-22 |
| Phase 10.2 — Admin Three.js Web 3D Viewport & Facility Page | Adds Three.js Web 3D Viewport component (`apps/admin/src/facilities/digital-twin-viewer.tsx`) rendering procedural refinery layout (distillation towers, storage tanks, pipe racks, pump skids) with spatial hotspot markers, color-coded status halos, Raycasting camera controls, camera preset smooth lerping, 3D-to-2D projected DOM tags, and spatial asset drawer. Adds protected route page (`apps/admin/src/app/(protected)/digital-twin/page.tsx`) and navigation drawer entry under Operations. 4 Vitest unit tests passed. | 2026-08-22 |
| Phase 10.3 — Mobile Client 3D Digital Twin Experience & Route | Adds Flutter `DigitalTwinScreen` (`apps/mobile/lib/facilities/digital_twin_screen.dart`), custom spatial node viewport painter, camera preset selectors, status filter chips, and asset bottom sheet. Registers `/digital-twin` route in `app_routes.dart` and `nav_config.dart` gated by `facilities.read`. 3 Flutter mobile tests passed. | 2026-08-22 |
| Phase 10.4 — Dashboard Integration & Full System Regression | Integrates 3D Digital Twin into admin navigation. Full multi-suite system regression passed: 438 API pytest tests, 212 admin vitest tests, and 283 mobile flutter tests. Phase 10 is **COMPLETE**. | 2026-08-22 |
| Phase 12.1 — public marketing site | Adds an unguarded `(marketing)` route group owning `/`, `/pricing`, and `/about`, rendered as static server components so they index without the browser-only Firebase auth context that gates the app shell. `src/navigation/routes.ts` becomes the single source for `APP_HOME`, the marketing/auth route lists, and the private prefixes robots.txt excludes; the signed-in dashboard moves from `/` to `/dashboard` and every redirect (post-login `next` fallback, verify-email exit, 403/404/coming-soon "back" affordances, PWA `start_url`) targets that constant instead of a `/` literal. The public header is the only client component in the group: it reads auth status purely to swap Log in / Start free trial for Go to dashboard, and renders no CTA while the session is restoring. `publicRoutes` now spans marketing plus auth pages, robots.txt disallows explicit app prefixes rather than a blanket `/`, and the sitemap ranks the landing page first with its root URL matching the page's own canonical form. Pricing tiers live in `src/marketing/pricing-plans.ts` with commercials explicitly flagged as unset placeholders. | 2026-09-07 |
| Phase 12.2 — marketing visual pass | Moves the public site's primary action to brand orange (`accent-500 → accent-600` gradient on `accent-ink`), revising D-016's rare-accent guidance for marketing surfaces only; the app shell keeps navy actions. Motion is added without giving up server rendering: sections carry a `data-reveal` attribute and one `RevealOnScroll` observer in the group layout adds `is-visible`, with the hidden state CSS-gated behind a `js-anim` class set by the pre-paint bootstrap so a no-JS visitor sees fully visible content instead of `opacity: 0`. `ScrollProgress` writes a single rAF-throttled `scaleX` transform to a 2px bar on the header's bottom edge, and `CountUp` animates the honest platform figures from a server-rendered final value. New `mk-*` classes in globals.css own the CTA sheen, panel hover hairline, ambient hero washes, gradient text, and — importantly — translucent surfaces via `color-mix`, because the Tailwind config maps `surface`/`background`/`elevated`/`border` to bare `var(--color-*)` references that its opacity modifier cannot alpha-blend, so `bg-surface/70` and friends compiled to nothing and left elements transparent. Every motion rule is disabled under `prefers-reduced-motion`. | 2026-09-07 |
| Phase 12.3 — marketing navigation fix and header layout | Fixes a blank-page regression introduced with 12.2's reveals. App Router keeps a layout mounted across navigations within its own group, so `RevealOnScroll`'s mount-only effect observed the landing page's elements and never those of `/pricing` or `/about`; with the hidden state applied by CSS, those pages rendered as a working header above an empty grid, and returning to `/` failed the same way because React had created fresh nodes. The effect now depends on `usePathname()` and additionally watches `<main>` with a childList MutationObserver, so any element mounted after the route commit is picked up; only childList is observed, so adding `is-visible` cannot retrigger it. `ScrollProgress` takes the same pathname dependency because a client navigation resets scroll and changes document height without firing a scroll event. Two regression tests cover both paths — the jsdom fallback and a stubbed IntersectionObserver with off-screen rects for the real browser path — and both fail against the pre-fix implementation. The header moves to a three-column grid so the site nav is genuinely centred, and the design-system's labelled `ThemeSwitch` pill is replaced in this header by a 40px icon-only toggle whose accessible name still states the theme it switches to. | 2026-09-07 |
| Phase 12.4 — landing page rebuilt around the product | Replaces the generic hero card and three-column problem/feature sets with depictions specific to this platform. `HeroShowcase` renders a stylised field device mid-inspection at Tank Farm A with no connection, its outbox draining, an SVG packet travelling a dashed wire, and the supervisor panel lighting up on receipt — one 9s CSS/SMIL loop, no client component, carrying `role="img"` plus a label that describes the illustration rather than implying a screenshot. `RecordChain` publishes the real sequence (INS-2291 finding → WO-1042 assigned work → PTW-118 hot-work permit → separate `work_orders.close` sign-off → RPT-0043 attested export) with the acting role and gating permission at each step, its connector drawing itself off the parent's reveal. `RoleMatrix` publishes all seven built-in roles with their real permission-key counts from `SYSTEM_ROLE_TEMPLATES` and the separation-of-duties notes the API enforces — deliberately shown pre-sales. The module grid becomes a bento layout with two wide tiles carrying real demo-tenant chips. `CountUp` was removed with the figure strip it served. Every animation has an explicit `prefers-reduced-motion` rest state: connected, queue drained, record received. | 2026-09-07 |
| Phase 12.5 — marketing action-cluster restyle | `CtaLink` moves to pill geometry (`rounded-full`) and gains a third `bare` variant plus an `arrow` prop. The header now reads as one primary action rather than a toolbar: a borderless glyph theme toggle with only a hover halo, "Log in" as an unboxed text action, and the orange primary as an arrow-free pill; body CTAs keep the arrow, which is the `arrow` prop's default for the accent variant. The primary keeps navy `accent-ink` rather than the reference's white, because white on the vivid `#FB4402` measures roughly 3.4:1 and fails AA at this text size, while the navy pairing is the one D-017 locked and clears 5:1. Note that the header's CTA and bare login are client-gated on settled auth status, so neither appears in server markup — the design contract for them is pinned by jsdom tests asserting the pill has no arrow child and neither the toggle nor the login link carries a border class. | 2026-09-07 |
| Phase 13.1 — plan catalog and entitlement enforcement | Adds `apps/api/app/billing/` as the authoritative commercial layer. `plans.py` holds the four published tiers (§22.1) with prices in cents, quotas (facilities/assets/seats), and a `Feature` set per tier; it is the single source the API enforces against, the portal renders from, and the Stripe Prices will be created from. `Company` gains `subscription_status`, `billing_interval`, `trial_ends_at`, `current_period_end`, and the two Stripe ids, all defaulted so every pre-Phase-13 document still loads as `incomplete` (granting nothing). `entitlements.py` resolves a company document to an `Entitlements` value with no I/O, so the dependency, the dashboard, and platform admin all resolve identically; `PAST_DUE` deliberately still grants access because locking an operator out of live permit and safety records over a failed card is unsafe, while `CANCELED` and `INCOMPLETE` grant nothing. `dependencies.py` adds `require_feature`, which stacks beside `require_permission` and shares its `access.denied` audit action with `gate="entitlement"`, returning **402** with the tier that unlocks the module rather than a bare 403 — a permission failure is final, an entitlement failure is purchasable. Quotas are checked in services via `assert_within_quota` because only the service can count the tenant's rows. `_audit_denial` became public `audit_access_denial` so both gates share it. | 2026-09-08 |
| Phase 13.2 — published pricing corrected | Replaces the placeholder commercials on `/pricing` with the requirements document's actual figures, mirroring the backend catalog: four tiers, per-seat add-ons (§22.2), capacity/module add-ons (§22.3), and implementation/support services (§22.4). Every trial CTA now carries `?plan=<tier>&interval=annual` so signup step two knows which Price to open. Monthly-billing prices are flagged in both catalogs as derived (list × 1.15) and pending owner sign-off, since the document gives only the annual-equivalent price plus a "~15% premium". | 2026-09-08 |
| Phase 13.3 — Stripe gateway, price sync, and webhook reconciliation | `app/billing/stripe_gateway.py` is the only module that imports Stripe; everything else works on plain values, so the checkout service and the reconciler test without a key or a network. Prices are addressed by deterministic **lookup key** (`fev_operations_annual`) rather than stored ids, so no `price_...` is committed and the same code path serves test and live keys. `scripts/stripe_sync.py` creates the four Products and eight Prices from the plan catalog and is idempotent — Prices being immutable, a changed amount archives the old one and recreates it under the same lookup key; `--verify` reads every key back and checks the amount against the catalog. `app/billing/service.py` reconciles webhooks by **re-reading the live subscription** rather than trusting the event body, which makes redelivery idempotent and out-of-order delivery harmless at the cost of one extra Stripe read. The tier is taken only from our own `fev_tier` metadata; an unlabelled subscription keeps the company's existing tier rather than guessing from the price. `app/api/v1/billing.py` exposes a public `/catalog`, an authenticated `/subscription` readable by every role (the shell needs it to decide which modules to render), a `company.settings`-gated `/checkout`, and an unauthenticated `/webhook` whose only gate is signature verification. The webhook answers 200 for verified-but-ignored events so Stripe stops retrying, and lets a reconciliation failure surface as 500 so it does retry. `error_responses` gained 400, 402, and 503. | 2026-09-08 |
| Phase 13.4 — two-step signup with Stripe checkout | Splits registration from payment. Step 1 is the existing details form; step 2 (`/signup/plan`) and the Stripe return page (`/signup/complete`) live in a new `(billing)` route group guarded by `RequireAccount`, which admits anyone whose account exists — `authenticated` **or** still unverified. That carve-out is the phase's central decision (D-092): a company created seconds ago has an unverified admin, and `require_billing_admin` on `POST /billing/checkout` deliberately drops `require_verified_email` while keeping `company.settings`, so paying is not blocked behind a mailbox round trip. `get_entitlements` was loosened to `get_current_user` for the same reason — the completion page polls the subscription while unverified — which is safe because every module gate stacks `require_permission`, and that still enforces verification. `PublicOnly` now sends an unverified user to the billing step rather than `/verify-email`, carrying any `?plan=` chosen on the pricing page through registration; the plan step forwards to `/verify-email` once a subscription exists, so a returning unverified user still lands correctly with no race. The picker renders from `GET /billing/catalog` rather than a client copy, so it cannot offer a tier the server will not sell, and prices come from the catalog's cents values. `SignupCompleteScreen` polls `GET /billing/subscription` because Stripe redirects the browser back before it has necessarily delivered `checkout.session.completed` — the redirect is not the confirmation — and offers both a re-check and a way out if the webhook never arrives. Contracts were regenerated for the three billing operations and `BillingApiClient` joined the `useAuth().apiClient` intersection. | 2026-09-10 |
| Phase 13.5 — plan-aware admin shell and dashboard | `SubscriptionProvider` loads `GET /billing/subscription` once inside `RequireAuth` and outside `AppShell`, and is the only client-side source of entitlements — the shell never derives them from the tier name, which would put the tier -> feature mapping in two places (D-093). `hasFeature` answers `false` while loading and on error, matching the API's 402: showing modules first and retracting them would flash capabilities a Starter tenant does not have. `NavItem` gained `requiredFeature` and `visibleNavGroups` now takes a second predicate, so an item needs both the person's permission and the company's plan; the two are independent by design and the default predicate allows everything so pre-Phase-13 callers keep their behaviour. Work Orders and Permits carry the Operations-and-above keys, so a Starter or Field tenant never sees them even though its Company Admin holds the permissions. The dashboard's raw-tier `StatusPill` is replaced by `PlanCard`: plan name, billing status (with a past-due warning that explicitly does not imply lockout, since access continues while Stripe retries), trial countdown, and usage bars per quota — limits from the API's resolved entitlements, unlimited rendered as text rather than an empty bar, and an unknown count shown as a dash rather than a fabricated zero. Four existing test harnesses (shell, dashboard, auth-experience, plus the new billing ones) now supply a subscription, because `useSubscription` throws without a provider — fail-closed was chosen over a permissive default, since silently allowing every feature if the provider were forgotten would defeat the gate. | 2026-09-10 |
| Phase 13.6 — plan-aware mobile app | `lib/billing/subscription_controller.dart` mirrors the portal's `SubscriptionProvider` deliberately, so both clients answer "may this company use this module" the same way: the feature set comes from `GET /api/v1/billing/subscription` verbatim, and `hasFeature` fails closed while loading and on error (D-093). `SessionSubscriptionScope` mounts one controller per company inside `RequireAuthGuard` and above the shell, keyed on `companyId` so one tenant's plan cannot persist into another's session, and refreshes in a post-frame callback so the first paint is never blocked on the network. `NavDestination` gained `requiredFeature`, and `AppNav.visible`/`primaryDestinations`/`secondaryDestinations` take an optional feature predicate defaulting to allow-all — so plan gating reaches the bottom navigation bar, not just the "More" sheet, and pre-Phase-13 callers are unaffected. `ApiContract` gained `getSubscription`, which required adding the method to all 18 test fakes; a shared `test/support/subscription_fixtures.dart` returns a fully-entitled Enterprise response so suites that are not about billing keep asserting on every module. | 2026-09-10 |
| Phase 13.7 — the entitlement gate applied | Closes a real gap: 13.1 defined `require_feature` and 13.5/13.6 hid modules in both clients, but the dependency was attached to **no route**, so the server half was decorative and a Starter tenant could still call `/api/v1/work-orders` directly. The gate is now a router-level dependency on all twelve module routers — assets/facilities/areas/qr (`assets`), inspections/checklist-templates (`inspections`), work-orders (`work_orders`), permits/permit-templates (`permits`), safety-reports, generated-reports, documents — and stacks with each route's own `require_permission`. Auth, billing, dashboard, users, roles, company, and audit stay ungated on purpose: an unpaid company must still be able to see why it is blocked and fix it. The seed's companies carried `subscription_tier="demo"`, which was never a published tier and resolved to no entitlements, so Acme is now `enterprise`/`active` and Beta is deliberately `starter`/`active` — the second tenant already proved multi-tenant isolation and now demonstrates plan gating with the seeded logins. A new `tests/conftest.py` makes every test an entitled Enterprise tenant by default, since the gate otherwise resolves a company through Firestore that the suite does not have; `test_billing_gate.py` overrides it per-tier and asserts the 402. | 2026-09-10 |
| Phase 13.8 — browser signup fixed at the generator | `POST /api/v1/auth/register` answered 422 for every browser attempt while the identical body sent by curl returned 201. Cause: openapi-generator emits a `...value` spread in `*ToJSON` for any schema declaring `additionalProperties: false`, reading the keyword as "this model carries extra properties" and passing the source object through verbatim — so the body held both `companyName` and `company_name`. That keyword comes from the API's `StrictModel` (`extra="forbid"`), which then rejected the duplicates. Exactly three of 185 models were affected, and they were precisely the three strictest ones: `CompanyRegistrationRequest`, `CompanyRegistrationResponse`, `CurrentUser`. Only signup actually broke, since the other two are responses and use `FromJSON`. Fixed in `packages/contracts/scripts/gen-clients.mjs`, which now strips the spread after generation — the clients are regenerated, so a hand-edit would not survive. `apps/admin/src/api/serialization.test.ts` asserts the registration body is exactly the four declared wire fields with no camelCase leak, and was confirmed to fail when the spread is reintroduced. | 2026-09-10 |
| Phase 13.9 — plan-aware widgets, sidebar plan footer, subscription page | Closes the gap 13.5 left: the nav hid Work Orders but the **dashboard widget** for it still called the gated `/api/v1/work-orders` route, so a Starter tenant saw a 402 toast on its own dashboard. `DashboardWidget` gained `requiredFeature` and the grid filters on it beside the permission, same contract as the nav (D-093); the pre-Phase-13 `minTier` mechanism is left in place but documented as superseded. `SidebarPlanSummary` replaces a footer that only offered "Collapse": plan, billing status, trial countdown and seat/asset usage bars, with collapse demoted to an icon beside it and a tooltip-carrying dot on the collapsed rail. It reads counts through the same cache keys the dashboard uses, so on that page it costs no extra request. `/settings/subscription` is the full view — plan, list price, renewal or trial end, support tier, live usage with remaining counts and at-limit copy, and included/not-included module lists rendered from the subscription's own `features` intersected with the catalog rather than a client-side tier lookup. `SubscriptionProvider` now reads `AuthContext` directly instead of `useAuth`, so a seeded provider needs no Firebase gateway and widget tests stay light. `ThemeToggleIcon` moved from the marketing chrome into the design system and is used by both headers, positioned left of the action cluster. The sidebar's `bg-surface/80` — another non-compiling alpha-on-CSS-var class — became a solid `bg-surface`, and `main` gained a thin token-coloured scrollbar rather than hiding a real affordance. | 2026-09-10 |
