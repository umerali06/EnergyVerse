...
void main() {
  testWidgets('shows loading then renders the company role catalog',
      (tester) async {
    final api =
  ...
  testWidgets('shows an honest empty state when no roles exist',
      (tester) async {
    final api = FakeApi(
  ...
  testWidgets('opens a role detail sheet with its permission set',
      (tester) async {
    final api = FakeApi(
      identityFor('company_admin', roleMatrix['company_admin']!),
  ...
  testWidgets('hides Roles for a role without roles.manage at the route level',
      (tester) async {
    final api =
