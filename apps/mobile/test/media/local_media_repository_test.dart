...
void main() {
  ...
  setUp(() {
    tempDir =
        Directory.systemTemp.createTempSync('fev_local_media_repository_test');
  });
  ...
  test('inspectionMediaStoragePath mirrors the backend object_path() formula',
    ...
    expect(path,
        'companies/acme-energy/inspections/insp-1/media/local-1_my_photo.jpg');
  });

  test(
      'inspectionVoiceNoteStoragePath mirrors the backend voice_object_path() formula',
      () {
    ...
    expect(path,
        'companies/acme-energy/inspections/insp-1/voice/local-1_note.m4a');
  });

  test(
      'enqueueCapture with kind "audio" stores durationMs and the voice/ storage path',
      () async {
    ...
    final row = await (db.select(db.mediaQueue)
          ..where((t) => t.localId.equals(localId)))
        .getSingle();
    expect(row.kind, 'audio');
    expect(row.durationMs, 42000);
    expect(row.storagePath,
        'companies/acme-energy/inspections/insp-1/voice/${localId}_note.m4a');
  ...
  test('a queued voice recording persists across an app restart', () async {
    ...
    final rows =
        await repository.dueForUpload(now: DateTime.utc(2026, 1, 1, 1));
    expect(rows.map((r) => r.localId), [localId]);
    expect(rows.single.durationMs, 9000);
  });

  test('enqueueCapture inserts a queued row in MediaQueue, not Outbox',
      () async {
    final db = AppDatabase(NativeDatabase.memory());
    addTearDown(db.close);
    final repository = LocalMediaRepository(db: db);
    ...
    expect(row.storagePath,
        'companies/acme-energy/inspections/insp-1/media/${localId}_photo.jpg');
    expect(row.gpsLat, 29.1);
    expect(await db.select(db.outbox).get(), isEmpty);
  });

  test('watchMediaForInspection only returns rows for that inspection',
      () async {
    final db = AppDatabase(NativeDatabase.memory());
    addTearDown(db.close);
    final repository = LocalMediaRepository(db: db);
  ...
  test(
      'removeBeforeSync deletes the local file and the row, and never touches Outbox',
      () async {
    final db = AppDatabase(NativeDatabase.memory());
    addTearDown(db.close);
    final repository = LocalMediaRepository(db: db);
  ...
  test('setBeforeAfterTag and setChecklistItemId update the row in place',
      () async {
    final db = AppDatabase(NativeDatabase.memory());
    addTearDown(db.close);
    final repository = LocalMediaRepository(db: db);
  ...
  test(
      'dueForUpload respects the backoff window, and bypassBackoff respects pausedSentinel',
      () async {
    final db = AppDatabase(NativeDatabase.memory());
    addTearDown(db.close);
    final repository = LocalMediaRepository(db: db);
    ...
    expect(await repository.dueForUpload(now: now, bypassBackoff: true),
        hasLength(1));

    await repository.markFailed(localId,
        message: 'permission-denied', nextAttemptAt: pausedSentinel);
    // A paused row is skipped even when bypassing ordinary backoff.
    expect(
        await repository.dueForUpload(now: now, bypassBackoff: true), isEmpty);
  ...
  test(
      'markReferenced deletes the row -- the synced inspection.media[] is now the source of truth',
      () async {
    final db = AppDatabase(NativeDatabase.memory());
    addTearDown(db.close);
    final repository = LocalMediaRepository(db: db);
