...
void main() {
  ...
  test(
      'a successful upload marks uploaded and enqueues attach_media onto the inspection outbox',
      () async {
  ...
  test('progress events update MediaQueue.uploadedBytes before completion',
      () async {
    final db = AppDatabase(NativeDatabase.memory());
    addTearDown(db.close);
    final api = FakeSyncApi();
    ...
    var row = await (db.select(db.mediaQueue)
    ...
    row = await (db.select(db.mediaQueue)
          ..where((t) => t.localId.equals(localId)))
        .getSingleOrNull();
  ...
  test('a retryable Storage error backs off instead of pausing', () async {
    ...
    final uploader = FakeMediaUploader(
      onUpload: (path, file, contentType) {
        ...
        scheduleMicrotask(
            () => upload.fail('retry-limit-exceeded', 'temporary'));
        return upload;
      },
    );
    ...
    expect(
        await mediaRepository.dueForUpload(
            now: now.add(const Duration(hours: 1))),
        hasLength(1));
  });

  test('a non-retryable Storage error pauses the row for manual retry',
      () async {
    final db = AppDatabase(NativeDatabase.memory());
    addTearDown(db.close);
    final api = FakeSyncApi();
    ...
    final uploader = FakeMediaUploader(
      onUpload: (path, file, contentType) {
        final upload = FakeMediaUpload();
        scheduleMicrotask(
            () => upload.fail('unauthorized', 'permission denied'));
        return upload;
      },
    );
    final worker = buildWorker(
    ...
    expect(
        await mediaRepository.dueForUpload(
            now: DateTime.utc(2026, 1, 2), bypassBackoff: true),
        isEmpty);

    await mediaRepository.retryMediaItem(localId);
    expect(await mediaRepository.dueForUpload(now: DateTime.utc(2026, 1, 1)),
        hasLength(1));
  });
