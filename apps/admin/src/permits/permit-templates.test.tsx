...
describe("permit template admin", () => {
  it("lists real templates and filters by permit type", async () => {
    const listPermitTemplates = vi.fn(async () => ({ items: [template], nextCursor: null }));
  ...
  it("keeps template mutation controls hidden for read-only roles", async () => {
    render(<Harness permissions={["permits.read"]} apiClient={{ listPermitTemplates: vi.fn(async () => ({ items: [template], nextCursor: null })) }}><PermitTemplatesPage /></Harness>);
  ...
  it("creates an ordered template using a real tenant role", async () => {
    const createPermitTemplate = vi.fn(async () => ({ ...template, id: "pt-new", version: 1 }));
    render(<Harness permissions={["permits.read", "permits.write"]} apiClient={{ listRoles: vi.fn(async () => roles), createPermitTemplate }}><PermitTemplateFormPage /></Harness>);
  ...
  it("saves an edit with the loaded optimistic version", async () => {
    const updatePermitTemplate = vi.fn(async () => ({ ...template, version: 4 }));
    render(<Harness permissions={["permits.write"]} apiClient={{ listRoles: vi.fn(async () => roles), getPermitTemplate: vi.fn(async () => template), updatePermitTemplate }}><PermitTemplateFormPage templateId="pt-1" /></Harness>);
  ...
  it("validates checklist and approval labels before a write", async () => {
    ...
    render(<Harness permissions={["permits.write"]} apiClient={{ listRoles: vi.fn(async () => roles), createPermitTemplate }}><PermitTemplateFormPage /></Harness>);
    await screen.findByRole("heading", { name: "Create permit template" });
    const user = userEvent.setup();
