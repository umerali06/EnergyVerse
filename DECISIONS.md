...
| D-102 | The Firebase web API key leaves source, and the archive branch is closed not merged | **GitHub secret scanning flagged a Google API key in 09a11d6c: the dev project's Firebase web key, hardcoded as a `defaultValue` fallback in `firebase_options.dart`. A Firebase web key is a public project identifier rather than a credential — Google documents it as safe in client code, and access control is Firestore/Storage rules plus App Check — but an unrestricted Google key can still be billed against other APIs, this project uses Maps Platform, and a committed key trips scanning either way. The values now come from `--dart-define` with no fallback; `firebaseClientOptions` already threw a named StateError on an empty value, so an unset define fails loudly. Removal fixes future commits only: the key was public and must be rotated or restricted in the Google Cloud console, which a history rewrite would not achieve. A full scan of all 133 commits for Google/AWS/Anthropic/OpenAI/Stripe/Slack/GitHub-PAT/private-key/service-account patterns found no other real hit. Separately, the `backup/pre-4.5-wip-20260729` PR was closed rather than merged: an archive branch 57 commits behind main, whose merge would drag six-week-old generated contracts back over current ones (the reported conflict) and reintroduce that commit. Its one genuinely unmerged piece, the branded verification email, was ported onto main cleanly instead as `POST /api/v1/auth/verification-email`, returning `sent=false` for an already-verified address and 503 rather than 500 when SES is unconfigured.** | **RESOLVED — LOCKED** | 2026-09-11 |
| D-103 | Signup verifies the email before it takes a card, and the return from Stripe confirms itself | **The order is now details -> verify email -> choose a plan -> app, reversing D-092. Verification last made it read as optional and, because the only verification mail was Firebase's unbranded firebaseapp.com default, it frequently never arrived at all -- the admin app never called the SES endpoint that already existed, so `register`/`resendVerification` now send through `POST /auth/verification-email` and fall back to the provider only when SES is unconfigured. The verify screen re-checks on a timer because the link is usually opened in another tab. `PublicOnly` sends an unverified user to `/verify-email`, `RequireAccount` admits only verified accounts, and `VerifyEmailGate` hands the onward choice to the subscription so a new admin goes to the picker and a returning one to the app. Separately, the completion screen no longer waits on a webhook it may never receive: Stripe substitutes `{CHECKOUT_SESSION_ID}` into the success URL, and `POST /billing/checkout/confirm` reads that session back and applies the subscription from a live `retrieve_subscription` -- the same reconciliation the webhook performs, so the two are idempotent and interchangeable, and the webhook remains the authority for renewals and cancellation. A session whose `client_reference_id` is not the caller's company is refused, so a pasted session id cannot buy another tenant's plan. `Continue anyway` is gone and `RequireSubscription` now gates the app shell: an admin without an active plan is returned to the picker instead of a dashboard with empty nav and 402 on every request. `platform.admin` bypasses that gate, since platform staff are not a paying tenant. Verified end to end against real Stripe test-mode keys and the live Firebase project, which found two defects no existing test could see: `_subscription_response` had been left calling itself, so the confirm route answered 500 on every request while its service passed every unit test (`tests/test_billing_routes.py` added, exercising the billing routes as HTTP); and a *refused* SES send — credentials present but rejected — escaped as a 500, now `EmailDeliveryError` mapped to 502 `email_delivery_failed`, distinct from the 503 for an unconfigured deployment.** | **RESOLVED — LOCKED** | 2026-09-14 |
| D-104 | The product is renamed to Flacron Energy; internal identifiers keep the `fev` short code | **Every user-visible string moves from "Flacron EnergyVerse"/"FEV" to "Flacron Energy" — site metadata, marketing and auth copy, the app-shell breadcrumb, permission descriptions, the SES sender name and email templates, PDF/DOCX report headers, the OpenAPI title, both PWA manifests, and the Stripe product names. The short code becomes `FE`, matching the new mark. What deliberately does **not** change is anything that is a contract rather than a label: the `@fev/api-client` package name, `FevApiClient`, the `fev_*` Stripe price lookup keys, the `fev_company_id`/`fev_tier` subscription metadata, the `fev-api` container name and the `FEV_DEBUG` env var. Those are addressed by live Stripe prices and by subscription metadata already written against real tenants; renaming them would break billing for no user-facing gain, and a repo/package rename is a separate job. The admin's `site.baseUrl` fallback was also corrected from the never-registered `app.flacronenergyverse.com` to the `www.flacronenergy.com` production already serves, since that fallback is what canonical URLs and the sitemap use whenever `NEXT_PUBLIC_SITE_URL` is unset. Brand assets are regenerated from the owner's new masters into the same filenames, so only `public/brand` changes: the near-white ground is removed by un-premultiplying (alpha from distance to the ground, then F = (C-(1-a)B)/a) rather than by keying a colour to zero, which keeps the flat interiors exactly on brand and leaves clean edges instead of a white fringe; each lockup is trimmed to its own bounds, so `Logo`'s intrinsic ratios had to move with them or the new `FE` mark would render stretched.** | **RESOLVED — LOCKED** | 2026-09-14 |
| D-105 | The legal package is content, not pages; acceptance is a precondition of the account | **The five policies (privacy, terms, industrial-safety disclaimer, refund, cookies) are structured data in `apps/admin/src/legal/documents/`, rendered by one `LegalPage`. Prose in JSX could not generate a table of contents from real headings, could not guarantee the same notice text appears in a policy and as an in-app warning, and would let the "Last Updated" date drift per page. `LEGAL_VERSION` is stated once on each side (`legal-content.ts`, `app/legal/versions.py`) and is what an acceptance is recorded against, so a material change can be detected rather than assumed. **Acceptance is required at registration**, not recorded afterwards: the package requires the acknowledgment before account activation, so an account that exists without one would already be the thing the requirement forbids -- `CompanyRegistrationRequest` makes the three flags required fields and the service refuses a registration where any is false, before the company or auth user is created, so a direct POST cannot skip the checkbox. The record lives on the user document (never orphaned from the account it describes) and keeps a SHA-256 of the caller's address, never the address. The checkbox is never pre-checked and the submit button is disabled until it is ticked. **Cookie consent** stores its decision against the same version, so a republished policy re-asks; rejection sits beside acceptance on the banner rather than behind "Manage Preferences", strictly-necessary is shown as always-active rather than as a toggle that pretends to be a choice, and a footer control reopens the manager through a window event. **`POST /api/v1/contact` is deliberately unauthenticated** -- a prospect, a security researcher, or someone exercising a privacy right has no account -- so its protection is bounds plus an allow-list of the 26 published categories, and it stores nothing. Categories are validated server-side rather than typed as a Literal, to avoid generating a 26-value enum into the Dart client for a web-only form.** | **RESOLVED — LOCKED** | 2026-09-14 |
| D-106 | Transactional email goes over SES's SMTP endpoint, and the send gate asks "can anything send" rather than "is the API configured" | **`SmtpEmailSender` is preferred whenever `SMTP_HOST`/`SMTP_USER`/`SMTP_PASS`/`SMTP_FROM` are all set; the boto3 `SesEmailSender` remains as the fallback. Both are kept because they fail differently and a deployment may only have one: SMTP needs egress on the submission port, which some hosts block, while the API path needs nothing but HTTPS. SMTP is preferred where both exist because its credentials are scoped to sending and nothing else, whereas the API path needs AWS keys carrying whatever else that IAM user can do. The connection is opened per send rather than pooled -- an idle SMTP session is dropped by the server and the failure then surfaces on some later, unrelated send. Every gate that asked `ses_configured` now asks **`email_configured`**: notification email was gated on the API credentials, so an SMTP-only deployment would have skipped every notification silently while the transport worked. The message now carries its own `Message-ID` and `Date`: SES adds them when it accepts a message through the API, but over SMTP the sender is responsible, and several large providers treat a message missing either as a spam signal -- the difference between "it sends" and "it sends, into spam". Each way SMTP can refuse maps to its own code (`smtp_auth_failed`, `smtp_sender_refused`, `smtp_recipient_refused`, `smtp_unreachable`) rather than one generic failure, because the fixes are unrelated to each other.** | **RESOLVED — LOCKED** | 2026-09-15 |
| D-107 | Launch pricing: the quoted figure is the monthly price, every paid tier carries the core product, and Enterprise is sold by sales rather than by card | **Five tiers — Pilot $499, Starter $999, Field $1,999, Operations $4,999 monthly, Enterprise custom-quoted from a published $9,999 floor. Three structural choices, each reversing something the old catalog did. (1) **The quoted price is the monthly price**, and annual is ten months of it. The previous catalog published an annual-equivalent and charged ~15% more month-to-month, so a monthly buyer never saw the number on the page; now both figures appear on the plan card and again in checkout before payment. (2) **AR inspection and work orders move down to Pilot.** The brief for Pilot included them while they were Operations-and-above, which would have made Pilot → Starter an upgrade that *removed* modules; instead every paid tier carries the core product (assets, inspections, AI analysis, AR, work orders, safety, documents, reports, digital twin) and the ladder is capacity, permit-to-work at Operations, and VR/SSO/audit-export at Enterprise. A test asserts over the real tier order that no upgrade can ever remove a feature. (3) **Enterprise is not self-serve**: it has no `monthly_cents`, no `annual_total_cents`, and no Stripe Price. `start_checkout` refuses it before reaching Stripe, `stripe_sync` skips it, the pricing page and the picker route to `/contact?topic=enterprise`, and the published floor is rendered as "starting around" and never as an amount that could be charged — publishing a buyable Enterprise price at its own floor would sell those deals at their minimum by accident. Add-ons were cut from eleven line items to eight, and seats are no longer sold individually: more seats means the next tier. Founding-customer pricing rides on Stripe promotion codes, which `allow_promotion_codes=True` already permitted but nothing told the customer about.** | **RESOLVED — LOCKED** | 2026-09-15 |
